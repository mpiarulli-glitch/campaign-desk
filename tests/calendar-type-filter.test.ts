...
test("editorial calendar type filter", async () => {
  ...
  // Nothing selected is "show the whole calendar", including untyped rows.
  ...
  assert.equal(sendMatchesTypeFilter("social_post", ["social"]), true);
  assert.equal(sendMatchesTypeFilter("email_campaign", ["email"]), true);
  assert.equal(sendMatchesTypeFilter("crm_automation", ["sms"]), true);
  assert.equal(sendMatchesTypeFilter("blog_post", ["blog"]), true);
  assert.equal(sendMatchesTypeFilter("", ["untyped"]), true);
  assert.equal(sendMatchesTypeFilter("email_campaign", ["untyped"]), false);

  assert.equal(sendMatchesTypeFilter("blog_post", ["email"]), false);
  assert.equal(sendMatchesTypeFilter("social_post", ["video"]), false);
  assert.equal(sendMatchesTypeFilter("social_video_carousel", ["social"]), false);
  ...
  assert.equal(sendMatchesTypeFilter("social_post", ["social", "blog"]), true);
  assert.equal(sendMatchesTypeFilter("blog_post", ["social", "blog"]), true);
});
